...
class MarkdownExtraProcessorTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$file = reset(iterator_to_array($this->dic['finder']['find']->files()->in($this->fixDir)->name('post-markdownextra-1.markdown')));
		...
		$matcher3 = array('tag' => 'h1', 'content' => 'Header', 'attributes' => array('id' => 'header'));

		$this->assertTag($matcher1, $post->getContent(), 'MardownExtra processor properly parse markdown formatted text (italic).');
		$this->assertTag($matcher2, $post->getContent(), 'MardownExtra processor properly parse markdown formatted text (strong).');
		$this->assertTag($matcher3, $post->getContent(), 'MardownExtra processor properly parse markdown formatted text (id in h1).');
