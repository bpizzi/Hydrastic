...
class TexyProcessorTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$file = reset(iterator_to_array($this->dic['finder']['find']->files()->in($this->fixDir)->name('post-texy-1.texy')));

		$post = new Post($this->dic);
		$post->read($file)->clean()->parseMetas()->parseContent();

		$matcher1 = array('tag' => 'p', 'content' => 'Lorem', 'attributes' => array('style' => 'color:blue'));
		$matcher2 = array('tag' => 'h1', 'content' => 'Ipsum');

		$this->assertTag($matcher1, $post->getContent(), 'Texy! processor properly parse texy formatted text (style for p tag).');
		$this->assertTag($matcher2, $post->getContent(), 'Texy! processor properly parse texy formatted text (h1).');
